...
for($i=1;$i<count($campi);$i++)
       $query.=",".$campi[$i];
$query.= " FROM news WHERE lingua='0' ORDER by id DESC LIMIT ".$limit;
...
$query="SELECT COUNT(*) FROM newsletter WHERE lingua='0' ";
$dbms->Exec_Query($query);
...
for($i=0;$i<$n;$i++)
{
	...
	$testobase.="<tr><td><b>".$topics["en"][$arr["tipo"][$i]]." :</b><a href=\"".$arr['link'][$i]."\">".$arr['titolo'][$i]."</a></b>
	<br><font size=-1> 
...
$nltesto='      
	...
	<center><h2>* * *  FreeScience.info Newsletter Number '.$numero.'  '.date('j-m-y').' * * *</h2> </center>   <br>  
'.$testobase.'</table>';
$page="<center><h2>Newsletter</h2><br>";
$page.='<form action="inviaen.php" method=post>
<input type=text name="subject" value="FreeScience.info Newsletter Number '.$numero.'" size=80><br><br>
